<template>
  <div class="h-full flex flex-col">
    <div class="flex justify-between items-center mb-4 flex-shrink-0">
      <div>
        <h3 class="text-base font-semibold text-gray-900">BTC Price Trend</h3>
        <p class="text-sm text-gray-500">Bitcoin price history</p>
      </div>
    </div>
...
const updateChartData = () => {
  ...
  const chartData = props.data
    .filter(item => item.btcPrice > 0) // Filter out invalid prices
    .map(item => ({
      time: new Date(item.timestamp * 1000).getTime() / 1000,
      value: item.btcPrice
    }))

  console.log('BTC Chart Data:', chartData)
